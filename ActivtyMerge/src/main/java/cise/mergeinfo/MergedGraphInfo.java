...
public class MergedGraphInfo {
   ...
   */
    ...
    /*
      1：对融合方案片段中每个节点集合，在其对应类型的融合方案中，将其随机的放置到一个预融合节点集中
     */
        for (PartialMergeVertexSet vSet : partialMergeSolution.getAllVertexSets()) {
            StringType type = vSet.type;
            // 获得该类型的融合方案
            MergeSolutionForType mergeSolutionForType = mergeSolutionForTypeMap.get(type);
            // 将节点集合插入到该类型融合方案的某一个预融合节点集合中
            mergeSolutionForType.addVertexSet(vSet, partialMergeAntiSolution);
        }

    /*
      2：对于反融合方案片段中的每个节点对，将其插入到融合方案中适当位置
     */
        for (AntiMergePair pair : partialMergeAntiSolution.antiMergePairSet) {
            StringType type = pair.v1.type;
            MergeSolutionForType solutionForType = mergeSolutionForTypeMap.get(type);

            // 如果当前的两个节点都已经出现在对应类型的融合方案中，则已经放置完毕
            if (solutionForType.containsVertex(pair.v1) && solutionForType
                .containsVertex(pair.v2)) {
                continue;
            }

            // 如果v1没有出现在融合方案中，添加v1
            if (!solutionForType.containsVertex(pair.v1)) {
                solutionForType.addVertex(pair.v1, partialMergeAntiSolution);
            }

            // 如果v2没有出现，添加v2
            if (!solutionForType.containsVertex(pair.v2)) {
                solutionForType.addVertex(pair.v2, partialMergeAntiSolution);
            }
        }
